...
# Usage: . .\infra\log-aliases.ps1

# Simple function aliases for session logging
function log { 
    & "$PSScriptRoot\..\PromptTracking\session-log.ps1" @args 
}

function logs { 
    & "$PSScriptRoot\..\PromptTracking\session-log.ps1" -Section @args 
}

function logc { 
    & "$PSScriptRoot\..\PromptTracking\session-log.ps1" -CodeBlock @args 
}

function logt([string]$title) { 
    & "$PSScriptRoot\..\PromptTracking\session-log.ps1" -Section -Title $title 
}

function logp { 
    & "$PSScriptRoot\..\PromptTracking\session-log.ps1" -Section -Title "Prompt:" -H1 -Markers Prompt 
}

function logr { 
    & "$PSScriptRoot\..\PromptTracking\session-log.ps1" -Section -Title "Response:" -H1 -Markers Response 
}

# General note with markers (H1 + Note markers)
function logn {
    & "$PSScriptRoot\..\PromptTracking\session-log.ps1" -Section -Title "Note:" -H1 -Markers Note
}

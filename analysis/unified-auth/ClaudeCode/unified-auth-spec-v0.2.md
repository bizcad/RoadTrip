...
**Author**: Nick Stein + Claude (v0.2 synthesis update)
...
This v0.2 update synthesizes ZeroPoint-inspired protocol features into the RoadTrip control-plane model by adding cryptographic decision receipts, hash-chained evidence continuity, and constitutional (non-overridable) policy constraints for critical actions.
...
| **Decision Receipt** | A signed, tamper-evident record of an authorization decision and its resulting action outcome. |
| **Receipt Chain** | Hash-linked sequence of decision receipts enabling integrity checks and non-repudiation over time. |
| **Constitutional Policy** | Non-removable, non-overridable policy constraints that are evaluated before ordinary policy rules. |
| **Capability Chain** | Delegation lineage proving who granted what authority to whom across hops. |
| **External Verifier** | Independent actor/system that can validate receipt signatures and chain integrity without trusting the runtime. |
...
- Receipts are hash-chained to prior receipts in the same trust domain to provide tamper evidence.
- Receipt verification is separable from the control plane so third parties can independently validate claims.

### 3.9 Constitutional Constraints Before Business Policy
- Certain constraints are non-overridable (for example: deny unless attested for critical action classes).
- Constitutional constraints are evaluated before mutable policy bundles.
- Operators can add stricter controls, but cannot disable constitutional controls at runtime.
...
The existing 4-layer model is preserved and extended to handle all three principal types. Three new layers are added for delegation, risk, and constitutional constraints.
...
| **L7: Constitutional Guard** | Does this action violate non-overridable constraints? | Immutable policy checks | Yes |

### Layer Evaluation Order

```
L0 → L1 → L2 → L3 → L4 → L5 → L6 → L7 → APPROVED
...
### 5.8 Layer 7: Constitutional Guard (NEW)

**Purpose**: Enforce non-overridable controls that cannot be bypassed by role, delegation, or environment toggles.

```
L7 checks:
  if action in policy.constitutional.critical_actions:
    if policy.constitutional.require_attested_code and principal_type == "code":
      if principal.trust_level != "attested":
        return FORBIDDEN_L7("critical action requires attested code principal")

    if policy.constitutional.require_receipt_emission:
      if not receipt_pipeline_available():
        return FORBIDDEN_L7("receipt pipeline unavailable for constitutional action")

    if policy.constitutional.require_human_for_nonrecoverable and not context.human_approval_present:
      return PENDING_L7("constitutional HITL required for non-recoverable action")
...
8. **Chain-verifiable**. Delegation events can be serialized into capability-chain proof material for receipt inclusion.
...
| **Gate** | **Primary enforcement point.** Run L0-L7 evaluation. HITL approval if required. This is where business decisions become enforceable. |
| **Execute** | PEP enforces the Gate decision. Token bound to execution scope. Credential expires at end. Emit signed decision receipt for configured action classes. |
| **Evaluate** | Validate receipt signature and chain continuity, then compare actual actions vs. authorized scope. Flag drift. |
| **Evolve** | Update trust levels based on outcomes. Promote or demote principals. Adjust risk thresholds. Feed into telemetry. |

---

## 9. Audit, Telemetry, and Cryptographic Receipts Contract

Every authorization decision produces an audit record.

### 9.1 Decision Log + Receipt Record
...
decision_log:
  ...
  evaluation:
    layers_evaluated: [0, 1, 2, 3, 4, 5, 6, 7]
    layers_passed: [0, 1, 2, 3, 4, 5, 6, 7]
    layers_failed: []
    decision: "APPROVED"
    risk_score: 15
    hitl_required: false
  ...
  receipt:
    receipt_id: "rcpt-2026-03-04-00045"
    subject_decision_id: "dec-2026-03-04-001"
    issued_at: "2026-03-04T10:05:23.700Z"
    issuer_principal: "agent:auth-validator-v2"
    payload_hash: "sha256:9f8f..."
    previous_receipt_hash: "sha256:2ab1..."
    chain_scope: "repo:bizcad/RoadTrip"
    signature:
      algorithm: "ed25519"
      key_id: "did:key:z6Mk..."
      value: "base64:MEUCIG..."
    verification:
      status: "verified"
      verified_at: "2026-03-04T10:05:23.901Z"
      verifier: "service:receipt-verifier"
```
...
| `receipt_verification_failures` | Invalid signature or broken chain events | Tamper detection |
| `receipt_emission_latency_ms` | Time from decision to signed receipt persisted | Operational reliability |

### 9.3 Receipt Verification Requirements

1. Every receipt includes decision hash, previous receipt hash, signer identity, and signature algorithm.
2. Verification must be reproducible by an external verifier without privileged access to internal policy stores.
3. Any broken chain or invalid signature for constitutional action classes raises a security incident.
4. Receipt retention must meet or exceed audit retention for corresponding decision records.
...
### Phase 2c: Receipt Pilot (ZeroPoint Synthesis)
- Add signed decision receipts for high-risk classes (spend, deploy, policy change)
- Persist hash-linked receipt chain per trust domain
- Add independent receipt verification service and alerting
- Define minimum receipt schema and canonical hashing rules
...
- Introduce constitutional policy bundle with non-overridable controls
...
|-------|-----------------|
| `git-push-autonomous` | Full L0-L7 evaluation. Orchestrates sub-skill auth. |
| `auth-validator` | Becomes the PDP implementation. Extended with L0, L5, L6, L7. |
...
| `telemetry-logger` | Records L0-L7 decisions and receipt verification outcomes. Feeds Evaluate phase. |
...
| Decision logs tampered post-facto | Signed, hash-chained receipts expose tampering via verification failure. |
...
| Critical action executed while constitutional controls disabled | L7 enforces non-overridable controls before execution approval. |
...
| `unified-auth-policy-model.yaml` | YAML | Example unified policy configuration showing all 8 layers |
| `principal-capability-matrix.csv` | CSV | Quick-reference matrix: who can do what at each trust level |
| `decision-receipt.schema.json` (planned) | JSON Schema | Canonical structure for signed decision receipts and chain fields |
...
- ZeroPoint analysis: `data/Competition/zeropoint/roadtrip-vs-zeropoint-feature-matrix.md`

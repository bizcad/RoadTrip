...
`py scripts/generate_trust_scorecard.py --manifest-dir workflows/006-Skill-Acquisition/trust-manifests --release-id <release-id>`
...
This emits one `*.trust-manifest.json` file per skill in a format aligned with `TRUST_BUNDLE_TEMPLATE.json`.

Backward compatibility: `--bundle-dir` and `--bundle-evidence-map` still work as aliases.

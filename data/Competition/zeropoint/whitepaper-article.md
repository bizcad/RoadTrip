...
1. [Why This Exists — The Portable Trust Thesis]("#s0")
2. [Problem Statement]("#s1")
3. [Design Goals]("#s2")
4. [System Overview]("#s3")
5. [Receipts and Chains]("#s4")
6. [Governance Model]("#s5")
7. [Threat Model]("#s6")
8. [Transport Integrations]("#s7")
9. [Implementation Status]("#s8")
10. [Adoption Paths]("#s9")
11. [Roadmap]("#s10")
12. [Ethics, Non-Goals, and Misuse Resistance]("#s11")
13. [Conclusion]("#s12")
14. [Appendix A: Protocol Sketch]("#appendix-a")
15. [Appendix B: Glossary]("#appendix-b")
16. [Appendix C: Example Integration Pattern]("#appendix-c")
...
- **Your history is a [hash-chained]("https://en.wikipedia.org/wiki/Hash_chain") audit trail**, not a log file someone else controls. Tampering is detectable. Omission is provable.
...
- **Identity layer.** [Ed25519]("https://en.wikipedia.org/wiki/EdDSA#Ed25519") signing keys and [X25519]("https://en.wikipedia.org/wiki/Curve25519") key agreement. Identity is a keypair. Authentication is a signature.
- **Governance layer.** PolicyEngine with constitutional rules, composable operational rules, [WASM]("https://webassembly.org/")-extensible policy modules, and capability gating.
...
- **Transport layer.** Pluggable transport with multiple built-in integrations: Reticulum-compatible mesh ([HDLC]("https://en.wikipedia.org/wiki/High-Level_Data_Link_Control") framing, 128-bit destination hashing, link handshake), TCP client/server, UDP, and HTTP API. The governance primitives are transport-independent.
...
| Content Hash | `ch` | [Blake3]("https://en.wikipedia.org/wiki/BLAKE_(hash_function)#BLAKE3") hash of the action content |
...
2. Delegation depths increment [monotonically]("https://en.wikipedia.org/wiki/Monotonic_function") (0, 1, 2, ...).
...
The evaluation hierarchy enforces precedence. Constitutional rules win over everything. WASM modules can override the default allow but cannot override constitutional rules. The decision severity hierarchy is: Block(5) > Review(4) > Warn(3) > Sanitize(2) > Allow(1). The most restrictive decision always wins.
...
**GenesisKey** — self-signed root of trust, one per deployment. **OperatorKey** — signed by genesis, one per node operator. **AgentKey** — signed by operator, one per agent instance.
...
When two ZeroPoint nodes meet for the first time, the introduction protocol (`zp-introduction`) governs trust establishment. The initiator sends its certificate chain and a challenge [nonce]("https://en.wikipedia.org/wiki/Cryptographic_nonce"). The responder verifies the chain, builds a `PolicyContext` with `ActionType::PeerIntroduction`, and evaluates it against the policy engine. Same-genesis introductions are High risk; cross-genesis introductions are Critical. The policy engine decides — the protocol only generates the context.
...
| **Cross-operator trust failure** | One party can't trust another's agent outputs | `zp-introduction` protocol verifies certificate chains and evaluates peer trust through the policy engine (see §5.5) | Peer network discovery is out of scope; cross-genesis introductions require operator-configured policy |
| **"Security theater" governance** | Claim governance without real constraints | Constitutional rules are non-removable; explicit non-goals section; receipts are independently verifiable, not just logged | Some deployments may misuse branding while gutting constraints; [MIT/Apache-2.0]("https://opensource.org/licenses/MIT") allows this |
...
| **[Replay attacks]("https://en.wikipedia.org/wiki/Replay_attack")** | Resend messages or insert previously captured packets | MeshEnvelope sequence numbers (monotonic u64); 16-byte random nonces in link handshake; Ed25519 signatures over content hashes | Depends on peers tracking seen sequence numbers; long-offline nodes may have gaps |
| **Injection attacks** | Insert forged packets into mesh transport | HDLC framing with CRC verification; Ed25519 signature verification on all envelopes; link-level X25519 [ECDH]("https://en.wikipedia.org/wiki/Elliptic-curve_Diffie%E2%80%93Hellman") key agreement | Transport-level encryption depends on successful link establishment; unlinked broadcast packets are not encrypted |
| **WASM policy escape** | Malicious policy module attempts to break [sandbox]("https://en.wikipedia.org/wiki/Sandbox_(computer_security)") | [Wasmtime]("https://wasmtime.dev/") runtime with fuel limiting (configurable execution budget); hash verification of module contents before loading | Fuel exhaustion causes denial-of-service at worst; WASM sandbox escape would require a wasmtime vulnerability |
...
The most straightforward integration path. An [Axum]("https://github.com/tokio-rs/axum")-based HTTP server exposes the governance pipeline as a [REST API]("https://en.wikipedia.org/wiki/REST"). Agents communicate over standard HTTP/HTTPS — suitable for cloud deployments, container orchestration, and integration with existing web services. No mesh networking required.
...
- **HDLC framing** with [CRC-CCITT]("https://en.wikipedia.org/wiki/Cyclic_redundancy_check") verification, matching Reticulum's serial interface format.
- **128-bit destination hashing** using the same truncated [SHA-256]("https://en.wikipedia.org/wiki/SHA-2") scheme.
- **Ed25519 signing** and **X25519 ECDH** key agreement, matching Reticulum's cryptographic primitives.
- **500-byte default [MTU]("https://en.wikipedia.org/wiki/Maximum_transmission_unit")** with a 465-byte data payload — compatible with Reticulum's packet constraints and suitable for LoRa links.
...
| `zp-receipt` | Receipt type definitions and serialization for cross-crate portability |
| `execution-engine` | Governed execution runtime: sandboxed action execution with policy enforcement |
...
1. **Public repository release** — GitHub publish, [crates.io]("https://crates.io/") registration, CI/CD with cargo test + clippy + fmt + docs.
...
| **Guard** | Pre-action sovereignty check. Local-first, runs before the PolicyEngine, enforces the participant's right to refuse. |
...
| **GenesisKey** | Self-signed root of trust in the key hierarchy (one per deployment). All operator and agent certificates trace back to this key. Always held by a human. |
| **OperatorKey** | Signed by the GenesisKey. Represents a node operator in the key hierarchy. Can issue AgentKey certificates. |
| **AgentKey** | Signed by an OperatorKey. Represents a single agent instance. Carries a portable certificate chain for remote verification. |
| **Certificate Chain** | A sequence of signed certificates (genesis → operator → agent) that proves an agent's identity. Verified offline against six invariants. |
| **Introduction Protocol** | The governed handshake between two ZeroPoint nodes meeting for the first time — verifies certificate chains and evaluates peer trust through the policy engine. |
...
This pattern works identically regardless of whether the requester is a human operator, an autonomous agent, or an automated service. The GovernanceGate does not check what kind of entity is requesting — it checks the cryptographic chain. This is the practical meaning of participant-agnostic governance.

...
**Purpose**: Enable Claude to discover and invoke skills based on semantic context without explicit calling.
...
## Known Improvements This Enables

✅ **No more "python vs py" Windows issues** — git-push-autonomous normalizes via skills  
✅ **No more explicit PS1 script calls** — skills abstract the implementation  
✅ **No more token/credential dialogs** — auth-validator handles it transparently  
✅ **No more vague prompts failing** — semantic discovery finds right skill  
✅ **Blog posts can publish themselves** — blog-publisher + git-push cooperation  
...
Claude:
1. Reads this INDEX.md
2. Sees `git-push-autonomous` in quick reference
3. Loads `git-push-autonomous-SKILL.md` for interface
4. Optionally loads `-CLAUDE.md` for decision logic
5. Invokes the skill without you needing to say:
   - "use git-push-autonomous skill"
   - "python vs py for Windows"
   - "read the config files"

**Result**: Cleaner prompts, faster execution, fewer mistakes.
...
8. Claude will auto-discover on next context refresh
...
**Next Step**: Test semantic skill invocation with next prompt: "commit and push these changes"
